...
class OlalaSimpleImage {
    ...
    function output($format = null, $quality = null) {
        ...
        switch (strtolower($format)) {
            ...
            case 'png':
                ...
                break;
            default:
                $getimagefunc = "getimagesizefromstring";
                $info = (empty($this->imagestring)) ? getimagesize($this->filename) : $getimagefunc($this->imagestring);
                $mimetype = $info['mime'];
                unset($info);
                break;
        }
    ...
    function overlay($overlay, $position = 'center', $opacity = 1, $x_offset = 0, $y_offset = 0) {
        ...
        // Load overlay image
        if( !($overlay instanceof OlalaSimpleImage) ) {
            $overlay = new OlalaSimpleImage($overlay);
        }
    ...
    protected function get_meta_data() {
        ...
        if(empty($this->imagestring)) {
            ...
            $getimagefunc = "getimagesizefromstring";
            $info = $getimagefunc($this->imagestring);
        } else {
